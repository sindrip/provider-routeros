...
- **v0.26.0** — no migration, and **it does not fix what it claimed to.** The
  CRDs are byte-identical to v0.25.0; it clears `Computed` on
  `/interface/wireguard` `private_key`, which changes only the schema upjet
  plans against and, as it turns out, changes nothing observable.
  The bug it was aimed at is real and still open: a `Wireguard` resource with a
  declared `privateKeySecretRef` rewrites itself to the router forever, at
  reconcile speed, with a payload of nothing but `mtu` and `name`, changing
  nothing on the device and reporting `Synced` and `Ready` throughout — so the
  first symptom is the router's flash wearing. The plan compares a prior state
  of the literal `*****` against the real key resolved from the Secret, and
  those never converge. The theory was that `Computed` was what put the
  redaction there; clearing it left the diff byte-identical, so that was wrong.
  Where the redaction is written is not yet known — not upjet, the Terraform
  SDK, the upstream provider or RouterOS, all of which return or store the key
  intact.
  **Until it is understood, do not declare `privateKeySecretRef` on a
  `Wireguard` resource.** With no value in the configuration there is nothing to
  diff against and the loop cannot form. The cost is real and worth stating: an
  undeclared key is regenerated by a factory reset, which changes the
  interface's public key and breaks every client profile pinning it, so keep
  the key somewhere you can restore it from by hand.
