...
type Route struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.dstAddress) || (has(self.initProvider) && has(self.initProvider.dstAddress))",message="spec.forProvider.dstAddress is a required parameter"
