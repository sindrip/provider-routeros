...
func providerForRuntime() *schema.Provider {
	return withCommentSequence(withCertificateRemoval(withInterfaceIdentity(withFactoryIdentity(withCommentIdentity(withNameIdentity(injectRouterComment(injectRouterName(withoutPolicyExpansion(withoutMistypedEnums(withoutPhantomDefaults(routeros.Provider())))))))))))
}
...
func providerForGeneration() *schema.Provider {
	p := documentCommentSequence(injectRouterComment(injectRouterName(withoutMistypedEnums(routeros.Provider()))))
