...
func providerForRuntime() *schema.Provider {
	return withCommentSequence(withCertificateRemoval(withInterfaceIdentity(withFactoryIdentity(withCommentIdentity(withNameIdentity(injectRouterComment(injectRouterName(withoutPolicyExpansion(withoutMaskedDiff(withSensitiveFields(withRouterReadOnly(withoutMistypedEnums(withoutPhantomDefaults(routeros.Provider()))))))))))))))
}
...
func providerForGeneration() *schema.Provider {
	p := documentCommentSequence(injectRouterComment(injectRouterName(withoutMaskedDiff(withSensitiveFields(withRouterReadOnly(withoutMistypedEnums(routeros.Provider())))))))
